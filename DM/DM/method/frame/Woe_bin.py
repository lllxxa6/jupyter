...
# -*- coding: utf-8 -*-
"""
Created on Sat Nov 28 23:15:41 2020
...
from method.frame.Util import rep_blank_na, x_variable, check_y, check_const_cols, \
                              check_datetime_cols, check_cateCols_uniqueValues, \
                              check_print_step, check_breaks_list, check_special_values
                             
from method.frame.Info_value import iv_01, woe_01, miv_01
...
TREE 方法以 [-inf, inf) 为初始分箱，向初始分箱中添加 breakpoint，使得IV收益最高

# 实现逻辑：
woebin(...                  # 1、最外层包装，增加参数校验、日志、多线程方法
    woebin2(...             # 2、包装，整合TREE/Chimerge方法/等距方法
        woebin2_tree(...    # 3、TREE方法实现
              # 1、生成初始化分箱
              # 初始化分箱最大箱数为 1/init_count_distr 默认 50
            initial_binning = woebin2_init_bin(...)
              # 2、迭代 插入分箱点
              # STEP 0: 创建分箱 [-inf,inf)
              # STEP 1: 插入最优分箱点, 从初始分箱点中选择, 使得总IV提升最大
              # STEP 2: 检查 IV 提升比例是否超过 stop_limit(默认0.1) 或 分割箱数小于 bin_num_limit(默认8)
              # IF STEP 2 TRUE 则重复 STEP 1，否则终止分箱，返回最终结果
            woebin2_tree_add_1brkp(...initial_binning...)
                )
           )
    )
...
Chimerge方法 方法以 all breaks 为初始分箱，将低于临界值的箱，按卡方值由小到大进行合并分箱

# 实现逻辑：
woebin(...                      # 1、最外层包装，增加参数校验、日志、多线程方法
    woebin2(...                 # 2、包装，整合TREE/Chimerge方法/等距方法
        woebin2_chimerge(...    # 3、Chimerge方法方法实现
             # 1、生成初始化分箱
             # 2、为初始分箱添加卡方值
           binning_chisq = add_chisq(woebin2_init_bin(...))
             # 3、迭代
             # STEP 0: 创建初始分箱 = initial_binning
             # STEP 1: 为初始分箱计算卡方值
             # STEP 2: 将卡方值最小一箱与相邻箱进行合并
             # STEP 3: 重新计算每个分箱卡方值
             # 满足以下条件之一则继续执行，否则停止分箱：
             #     a 初始分箱中剩余最小卡方值 < chisq_limit(默认 2.7) 参考 https://docs.scipy.org/doc/scipy/reference/generated/scipy.special.chdtri.html
             #     b 初始分箱中剩余最小箱占比 < init_count_distr(默认 0.05)
             #     c 当前分箱数 > bin_num_limit(默认8)
           binning_chisq = binning_chisq.group_by(...)
                        )
           )
    )
...
def add_missing_spl_val(dtm, breaks, spl_val):
    ...
    '''
    # dtm 是一个由label(y)、当前待分箱特征名称与该特征值组成的dataframe
    # 该方法检查dtm中是否存在np.nan
    # spl_val 为声明的bin分割值，若原数据中存在缺失值，则分箱时需单独处理缺失数据，
    # 因此需要在 spl_val 中设置一个分割点，用于区分缺失值，这里默认为 missing
    # 因此，当存在缺失数据时该方法处理三种情况：
    # 1 当 spl_val 未声明时将 `missing`设为 spl_val=['missing']
    # 2 当 spl_val 声明且包含 `missing`时，无需处理 spl_val=spl_val
    # 3 当 spl_val 声明且未包含 `missing`，追加 ['missing']+spl_val
...
def dtm_binning_sv(dtm, breaks, spl_val):
    ...
    if spl_val is not None:
        ...
        if is_numeric_dtype(dtm['value']):
            # 将 sv_df.value 属性修改为 numeric
            # 改值有可能为object，如果在前一步split_vec_todf(spl_val)中替换了missing
        ...
        # binning_sv
        # 对 dtm_sv 进行分箱统计，dtm则直接返回（已过滤na与spl_val）
        binning_sv = pd.merge(
...
def woebin2_init_bin(dtm, init_count_distr, breaks, spl_val):
    ...
    while len(init_bin.query('(good==0) or (bad==0)')) > 0:
        ...
        init_bin = init_bin.groupby('brkp').agg({
          ...
          'good': sum,
          'bad': sum
        }).assign(badprob = lambda x: x['bad']/(x['good']+x['bad'])).reset_index()
...
def woebin2_tree(dtm, init_count_distr=0.02, count_distr_limit=0.05, 
    ...
    Params
    ------
    dtm: a melt dataframe with y, x_name, x_values as columns
...
def woebin2(dtm, breaks=None, spl_val=None, 
    ...
    Params
    ------
    dtm: melt dataframe with 3 columns, label、variable name and values;
    
    other parameters check at `woebin` function
...
def woebin(dt, y, x=None, 
    ...
    var_skip: 跳过分箱的变量, skip for bining; 
        计算变量woe，但不执行 optimal bin process;

    breaks_list: 指定变量分箱策略(break points);
        未指定 breaks_list 的变量，由 process 提供 bin 策略;
    
    special_values: the values specified in special_values 
        will be in separate bins. Default is None.
    
    count_distr_limit: 指定分箱策略单一的bin的占比不低于 count_distr_limit;
        min( % bin of total ) >= count_distr_limit;
        默认 0.05; 建议范围 0.01 - 0.2;
    
    stop_limit: 指定 information value / gain ratio 值，当该值 < stop_limit 时，终止分箱过程；
        chi-square 方式下，min chi-square < 'qchisq(1-stoplimit, 1)' 时终止；
        默认 0.01; 建议范围 0.00 - 0.5;

    bin_num_limit: Inte 单个bin包含的最大样本量；

    no_cores: 并行 CPU 数量限制;
    ...
    # 排除 只包含唯一值的变量 len(dat[i].unique())==1
    ...
    if stop_limit<0 or stop_limit>0.5 or not isinstance(stop_limit, (float, int)):
        warnings.warn("[ 异常声明 ] accepted stop_limit parameter range is 0-0.5. Parameter was set to default (0.1).")
        stop_limit = 0.1
    # init_count_distr range
    if init_count_distr<0.01 or init_count_distr>0.2 or not isinstance(init_count_distr, (float, int)):
        warnings.warn("[ 异常声明 ]  accepted init_count_distr parameter range is 0.01-0.2. Parameter was set to default (0.02).")
        init_count_distr = 0.02
    # count_distr_limit
    if count_distr_limit<0.01 or count_distr_limit>0.2 or not isinstance(count_distr_limit, (float, int)):
        warnings.warn("[ 异常声明 ]  accepted count_distr_limit parameter range is 0.01-0.2. Parameter was set to default (0.05).")
        count_distr_limit = 0.05
    # bin_num_limit
    if not isinstance(bin_num_limit, (float, int)):
        warnings.warn("[ 异常声明 ]  bin_num_limit should be numeric variable. Parameter was set to default (8).")
        bin_num_limit = 8
    # method
    if method not in ["tree", "chimerge"]:
        warnings.warn("[ 异常声明 ]  method should be tree or chimerge. Parameter was set to default (tree).")
        method = "tree"
    ### ### 
...
def woebin_save(bins, route, vb_code=None, time_start=None):
    ...
    if vb_code is not None:
        if not isinstance(vb_code, dict):
            raise Exception(' variable declare not right')

    filename = '{}/Woebins_{}.xlsx'.format(route, t)
    wid_num = 10
    wid_var = 18
    wid_str = 25
