...
# -*- coding: utf-8 -*-
"""
Created on Sat Nov 28 23:44:42 2020
...
def perf_psi(score, fig_size=(10,6), fill0 = 0.001, label=None, title=None, x_limits=None, x_tick_break=50, show_plot=True, seed=186, return_distr_dat=False):
    '''
    PSI
    '''
    ...
    dt_sl = pd.concat(score, names=['ae', 'rowid']).reset_index().sample(frac=1, random_state=seed)
    # ae refers to 'Actual & Expected'
    
    # PSI function
    def psi(dat):
        dt_bae = dat.groupby(['ae','bin']).size().reset_index(name='N')\
          .pivot_table(values='N', index='bin', columns='ae').fillna(fill0)\
    ...
    for sn in score_names:
        ...
        distr_prob = dat.groupby(['ae', 'bin'])['y'].agg([good, bad])
        distr_prob = distr_prob.fillna(0)
        distr_prob = distr_prob \
          .assign(N=lambda x: x.good+x.bad, badprob=lambda x: x.bad/(x.good+x.bad)).reset_index()
        ...
        if show_plot:
            ...
            #plt.xticks(ind+width/2, distr_prob.index, rotation=35)
            plt.xticks(ind+width/2, distr_prob.index, rotation=35)
